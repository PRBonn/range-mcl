...
void fast_rendering(vec4[3] positions, vec4[3] normals)
{
  // just check all cases.
  if((positions[0].y < 0 && !(positions[1].y < 0 || positions[2].y < 0)) ||
     (positions[1].y < 0 && !(positions[0].y < 0 || positions[2].y < 0)) ||
...
void main() {
  ...
  // // if meshes are small, x axis check is enough
  ...
  fast_rendering(vec4[3](gs_in[0].position, gs_in[1].position, gs_in[2].position), vec4[3](gs_in[0].normal, gs_in[1].normal, gs_in[2].normal));
}